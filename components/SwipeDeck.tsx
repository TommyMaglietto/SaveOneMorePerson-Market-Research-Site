...
const profanitySafeWords = new Set(["scunthorpe"]);
...
const containsProfanity = (value: string, filter: Filter) =>
  getProfanityTokens(value).some((token) => {
    const lower = token.toLowerCase();
    if (profanitySafeWords.has(lower)) {
      return false;
    }
    ...
    return filter.isProfane(lower) || filter.isProfane(normalized);
  });
